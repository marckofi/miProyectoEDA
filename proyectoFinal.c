...
/*
La mayoría de problemas que tuvimos se debía a que la lectura de datos así que 
hicimos funciones como leerEnteroRango leerEntero y leerFloat para que se pudieran 
leer correctamente los datos. 

Tuvimos un problema para agregarPedido pues para generar pedido estabamos usando una estructura con dos arreglos 
uno para las ids y otra para la cantidad, entonces cada vez que se generaba un pedido se usaba  esta estructura y se 
creaban los arreglos con malloc sin embargo tuvimos problemas con indices de los arreglos pues no se guardaba correctamente 
los productos que se solicitaban. Para arreglarlo cambiamos de idea e implementas un pedido como una lista doble donde los nodos 
eran productos, esto facilito mucho las cosas. 

También tuvimos problemas a la hora de agregarPedido y generar Venta pues si 
se acababa la existencia  mientras haciamos un pedido o una venta y se elegía agregar otro producto 
no había forma de salir así que hicimos una función existenciaInventario para saber si existe al menos un producto en el inventario 
y la usamos en agregarPedido y generar Venta entonces se evalua está función en un if por cada vez que se agrega un producto, entonces si 
no había existencias se sale del ciclo y ya no se agregan más productos y se quedan con los que se habían agregado. 
...
/*
    En el campo info es tipo void * porque esta estructura se usa para la lista de productos, pedidos y 
    para la cola de pedidos. Para lista de productos y para los pedidos generados en agregarPedido info es una dirección de una estructura Producto. 
    Para los nodos de colas de pedidos el campo info es una dirección de una lista doble. La estructura lista doble se usa para generar un pedido.
    Tuvimos cuidado de incluir varias conversiones de tipo(type cast) a la hora de trabajar con el nodo doble. 
*/
...
/*
Se usa max tamaño porque la pila es dinámica. 
Cada vez que se quiera hacer un push en la pila  se evalua si está llena. si sí entonces se redimensiona 
la pila
*/
...
while (pregunta == 1) {
    ...
    if (cantidad_producto_venta > 0) {
        ...
        if(producto_venta_auxiliar != NULL){
            producto_venta_auxiliar->cantidad += cantidad_producto_venta;
...
void mostrarSubMenuInventario(lista_doble * lista_productos) {
    ...
    while (1) {
        ...
        switch (opcion) {
            case 1:
                printf("\n ---Opcion: Ver Inventario--- \n");
                verInventario(lista_productos);
                break;
            case 2:
                printf("\n ---Opcion: Actualizar Inventario--- \n");
                actualizarInventario(lista_productos);
                break;
            case 3:
                printf("\n ---Opcion: Buscar en Inventario--- \n");
...
void mostrarSubMenuVentas(lista_doble * lista_productos, pila_ventas * pila_de_ventas) {
    ...
    while (1) {
        ...
        switch (opcion) {
            case 1:
                printf("\n ---Opcion: Registrar Ventas--- \n");
                registrarVentas(lista_productos, pila_de_ventas);
                break;
            case 2:
                printf("\n ---Opcion: Ver Ventas Realizadas--- \n");
